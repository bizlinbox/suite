...
router.get('/', async (req, res, next) => {
  try {
    const result = await query(
      'SELECT id, name, timezone, platform_name, platform_logo, created_at FROM organizations WHERE id = $1',
      [req.user.org_id]
    );
...
router.put('/:id', async (req, res, next) => {
  try {
    const { name, timezone, platform_name, platform_logo } = req.body;
    ...
    const result = await query(
      `UPDATE organizations
       ...
       WHERE id = $5
       RETURNING id, name, timezone, platform_name, platform_logo, created_at`,
      [name, timezone, platform_name, platform_logo, req.params.id]
    );
    if (result.rows.length === 0 || result.rows[0].id !== req.user.org_id) {
      return res.status(404).json({ error: 'Organization not found' });
    }
